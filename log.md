...
## [2026-07-27] update | memfault-workflow.md — wrong-workspace OTA payload incident + chsh-ag-memfault/chsh-sk-memfault fix
- Root-caused an OTA release (`v2.6.4.1`, nordic-wifi-memfault nrf7002dk) that downloaded cleanly on-device but never actually installed: `chsh-ag-memfault` had a hardcoded `Project root: /opt/nordic/ncs/v3.3.0/nordic-wifi-memfault`, so it uploaded a stale v3.4.0.x build from that other NCS-version workspace as the OTA payload instead of the intended v2.6.4.1 binary — same filenames/board target across workspaces made this silent.
- Confirmed by diffing the actual CDN-served OTA payload bytes (size/SHA256/embedded `strings` version) against the known-correct GitHub release artifact — cohort/deployment/API state all looked healthy the whole time.
- Fixed in place (not just documented): removed the hardcoded root from `~/.claude/agents/chsh-ag-memfault.md` (now requires an explicit absolute PROJECT_ROOT or artifact paths per task), added mandatory pre-upload `strings`-based version verification (abort on mismatch) and a post-upload CDN re-download re-verification step; updated `~/.claude/skills/chsh-sk-memfault/SKILL.md` to require the delegating prompt to always paste a literal resolved absolute path.
- Updated concepts/memfault-workflow.md with a new "Incident: Wrong-Workspace Binary Uploaded as OTA Payload" section capturing symptom → root cause → how it was found → fix.
...
- Diagnosed nordic-wifi-memfault v2.6.4.5 "device goes silent after random uptime, needs power cycle" — infinite loop in vendored Memfault SDK 1.6.0 `prv_read_socket_data()`: `recv()` returns 0 on orderly peer close and leaves `errno` untouched, so a stale `EAGAIN` makes end-of-stream look retryable; Zephyr keeps `POLLIN` asserted after a stream-socket peer close, so `poll()` never throttles the loop.
- Two amplifiers turned a stalled upload into an unrecoverable freeze: the SDK's periodic-upload work queue runs at `K_HIGHEST_APPLICATION_THREAD_PRIO` (starves the priority-14 log thread → UART goes silent), and `tls_heap_lock` was acquired `K_FOREVER` at every TLS site (froze MQTT/HTTPS/upload thread too).
- Key discriminator: graceful FIN/close_notify hangs; abrupt RST is handled correctly via POLLERR/ECONNRESET — which is why the failure time looks random.
- Cleared the "always close socket" patch as a suspect: `ztls_close_ctx()` → `tls_tx()` is unconditionally `ZSOCK_MSG_DONTWAIT`, so close_notify cannot block.
- Recorded in concepts/memfault-workflow.md (new incident section), including the 99.53 %-of-448 KB RAM ceiling that makes unmeasured stack bumps unaffordable on this project.
